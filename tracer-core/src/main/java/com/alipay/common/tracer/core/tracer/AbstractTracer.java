...
public abstract class AbstractTracer {
    ...
    public AbstractTracer(String tracerType, boolean clientTracer, boolean serverTracer) {
        ...
        Reporter reporter = this.generateReporter();
        if (clientTracer) {
            if (reporter != null) {
    ...
    protected Reporter generateReporter() {
        String digestRollingPolicy = SofaTracerConfiguration
            .getRollingPolicy(getReporterRollingKey());
        String digestLogReserveConfig = SofaTracerConfiguration
            .getLogReserveConfig(getReporterLogNameKey());
        // Construct a reporter instance
        DiskReporterImpl reporter = new DiskReporterImpl(getReporterLogName(), digestRollingPolicy,
            digestLogReserveConfig, getEncoder(), generateStatReporter(), getReporterLogNameKey());
        return reporter;
    }

    /**
     * To return digest report log name
     * @return
     */
    protected abstract String getReporterLogName();

    /**
     * To return digest report rolling key
     * @return
     */
    protected abstract String getReporterRollingKey();

    /**
     * To return digest report log name key
     * @return
     */
    protected abstract String getReporterLogNameKey();

    /**
     * To return digest report SpanEncoder
     * @return
     */
    protected abstract SpanEncoder<SofaTracerSpan> getEncoder();

    /**
     * To return stat report
     * @return
     */
